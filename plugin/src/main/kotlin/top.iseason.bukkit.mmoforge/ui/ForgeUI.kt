...
class ForgeUI(val player: Player) : ChestUI(
    ...
    init {
        ...
        ForgeUIConfig.slots["material"]?.forEach { (item, slots) ->
            ...
            for (slot in slots) {
                val materialSlot = IOSlot(slot, item2)
                materialSlots.add(
                    materialSlot.inputFilter {
                        if (inputData == null) return@inputFilter false
                        isValidForgeMaterial(it, materialSlot)
     ...
     * 更新界面
     */
    private fun updateMaterialSlots() {
        materialSlots.forEach { slot ->
            val itemStack = slot.itemStack ?: return@forEach
            if (inputData == null || !isValidForgeMaterial(itemStack, slot)) {
    ...
    private fun isValidForgeMaterial(item: org.bukkit.inventory.ItemStack, currentSlot: IOSlot? = null): Boolean {
        val nbtItem = NBTItem.get(item) ?: return false
        if (!nbtItem.hasType()) return false
        val allowedTypes = getAllowedForgeTypes()
        if (allowedTypes != null) {
            val type = nbtItem.type.lowercase()
            val id = nbtItem.getString("MMOITEMS_ITEM_ID")?.lowercase() ?: return false
            if ("$type:$id" !in allowedTypes) return false
        }
        val exp = nbtItem.getDouble(ForgeExp.stat.nbtPath)
        if (exp != 0.0) return true
        val chance = nbtItem.getDouble(ForgeChance.stat.nbtPath)
        if (chance == 0.0 || nbtItem.hasForgeData()) return false
        for (materialSlot in materialSlots) {
            if (materialSlot === currentSlot) continue
